...
def generate_article_topics_and_contents(category_name, location_name):
    ...
    def generate_article_content(topic):
        prompt = f"Write detailed article without title part and conclusion part on - {topic}"
        prompt_explain = "write from prespective of rental platform that has multiple listings of rentals"
        prompt = prompt + prompt_explain
...
def generate_faq(category_name, location_name):
    ...
    def get_faq_questions(category_name, location_name):
        ...
        prompt = f"Suggest 25 FAQ questions related to {category_name} in {location_name}\nI only need FAQ questions"
    ...
    def generate_faq_answers(question):
        # Construct the prompt
        prompt = f"Write detailed answer to the FAQ question: '{question}'"
        prompt_explain = "write from prespective of rental platform that has multiple listings of rental."
        prompt = prompt + prompt_explain
...
for category_name, category_id in category_data.items():
    for location_name, location_id in location_data.items():
        print("-------------------->", category_name, "-----", location_name)
        ...
        if access_token:
            ...
            time.sleep(1)  # Delay before sending the next requests to avoid rate limiting
            logging.info(f"Processed data : {location_name} -- {category_name}")  # Log the location
